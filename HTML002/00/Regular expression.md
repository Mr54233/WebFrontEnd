...
作用: 计算机可以根据正则表达式 , 来检查一个字符串是否符合指定的规则 ; 或者将字符串符合规则的内容提取出来.
...
-   创建正则表达式的对象 reg
-   使用 reg 的 test()方法, 判断指定字符串是否符合规则.
    正则表达式的 test()方法: 重要

```js
myReg.test(str); // 判断str字符串是否符合 , 指定的myReg这个正则表达式的规则
```

> 解释: 使用 test()这个方法可以用来检查一个字符串是否符合正则表达式的规则, 如果符合返回 true, 否则返回 false.
...
```js
var reg = new RegExp("a"); // 定义一个正则表达式 , 检查一个字符串中是否含有a
var str1 = "123456a";
var str2 = "123456";

// 通过test()方法 , 判断字符串是否符合上面定义的reg 规则
...
- `i` 忽略大小写 , 这个i 指的是 ignore .
- `g` 全局匹配模式 , 这个g 指的是 global . 
...
var 变量 = /正则表达式/匹配模式 ; // 注意 , 这个语法中没有引号
```
栗子:
```js
var reg =/A/i; // 定义正则表达式的规则 : 检查一个字符串中是否含有a , 忽略大小写;
var str = 'qiangu';
...
**备注**：如果在正则表达式中同时使用`^`和`$`符号，则要求字符串必须**完全符合**正则表达式。
